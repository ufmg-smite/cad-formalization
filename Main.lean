...
def r1 : AlgebraicNumber.Raw := ⟨p, 1.4, 1.5⟩
def a1 : AlgebraicNumber.AlgNum := by lift_alg_num r1
def r2 : AlgebraicNumber.Raw := ⟨p, -1.49, -1.4⟩
...
def r3 : AlgebraicNumber.Raw := ⟨p2, 7/5, 2⟩
...
def r1 : AlgebraicNumber.Raw := ⟨p1, -3/2, -5/4⟩
def R1 : AlgebraicNumber.AlgNum := by lift_alg_num r1

def r2 : AlgebraicNumber.Raw := ⟨p1, 1, 5/4⟩
...
def zero_r : AlgebraicNumber.Raw := ⟨zero_p, -1, 1⟩
